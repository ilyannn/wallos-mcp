...
export const getMasterDataTool: Tool = {
  ...
  description: 'Retrieve all master data from Wallos including categories, currencies, payment methods, and household members',
  inputSchema: {
    type: 'object',
    properties: {},
...
export async function handleGetMasterData(
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log(`Successfully retrieved master data: ${response.summary.categories_count} categories, ${response.summary.currencies_count} currencies, ${response.summary.payment_methods_count} payment methods, ${response.summary.household_members_count} household members`);
